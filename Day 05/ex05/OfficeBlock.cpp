...
{
	if (_signer != nullptr)
		throw(OfficeBlock::SignerAlreadySetException("we are not ready to fire the signer"));
	_signer = &signer;
}
...
{
	if (_executor != nullptr)
		throw(OfficeBlock::ExecuterAlreadySetException("we are not ready to fire the executor"));
	_executor = &executor;
}
